...
/**
 * Created by jiatieying on 2016/12/16.
 */
public class LuceneTest {
    // 待索引文件存放路径
    private static String source_file_path = "/Users/jiatieying/Downloads/Demo4Lucene";

    // 生成的全文索引文件存储路径
    private static String indexDir = "/Users/jiatieying/Downloads/LuceneIndexDemo";
    ...
    public void createIndex() {
        ...
        try {
            ...
            for (String txtFile : txtFiles) {
                ...
                // 以下生成各个字段的数据值
                StringField name = new StringField("filename",	input_txt_file.getName(), Field.Store.YES);
                TextField content = new TextField("content", readFileContent(	file, "gbk"), Field.Store.YES);
                StringField path = new StringField("path",input_txt_file.getAbsolutePath(), Field.Store.YES);
                StoredField date = new StoredField("date",	df.format(new Date()));
                LongField size = new LongField("size", input_txt_file.length(), Field.Store.YES);
